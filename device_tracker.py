...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_PING_TIMEOUT, default=2.0): vol.All(
...
class ArpDeviceScanner(DeviceScanner):
    ...
    def __init__(self, config):
        ...
        for mac in config[CONF_EXCLUDE]:
            self.exclude.append(mac.lower())
        for mac in config[CONF_INCLUDE]:
            self.include.append(mac.lower())
    ...
    def _update_info(self):
        ...
        if self.fping_interval and (
            self.fping_time is None or now > self.fping_time + self.fping_interval
        ):
            ip_masks = []
            single_ips = []
            ...
            for mask in ip_masks:
                cmd = [
                    ...
                    "0",
                    "-t",
                    str(int(self._ping_timeout * 1000)),
                ] + mask
        ...
        for line in out.splitlines():
            ...
            # _LOGGER.debug('FOUND %s %s %s', hostname, ipv4, mac)
            ...
            #         skip = True
            #         break
            if self.include and mac not in self.include:
                skip = True
            ...
            cmd = ["ping", "-n", "-q", "-c1", "-W" + str(int(self._ping_timeout))]
            if self._iface:
                cmd += ["-I", self._iface]
            cmd += [ipv4]
            _LOGGER.debug("Running " + " ".join(cmd) + "...")
